...
static int Pods(int result, long long int table, short StrNum, short Pos)
{
	for (short i = 0; i < 4; i++)
...
static short FourBitInNum(unsigned int vector, short Pos)
{
	short Num = 0;
	for (short i = 0; i < 4; i++)
		if (vector & (1<<(i+Pos*4)))
...
static int BlockPods(unsigned int vector)
{
...
static unsigned int BitShift(unsigned int num, int shift)
{
	return (num<<shift) | (num>>32-shift);
}

static unsigned int sumator2v32(unsigned int num1, unsigned int num2)
{
...
static unsigned int sumator2v32m1(unsigned int num1, unsigned int num2)
{
...
static unsigned int* TrueRand(int BitNum)
{
...
static void SimpleZamena(unsigned int* nakop1, unsigned int* nakop2, unsigned int* key)
{
	for (int i = 0; i<24; i++)
	{
